...
namespace A_Star_Planner {
    ...
    // Default constructor
    AStar::AStar() : initialized_(false), costmap_(nullptr) {}
    ...
    void AStar::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros){
        if(!initialized_){
            ...
            make_plan_srv = private_nh.advertiseService("make_plan", &AStar::makePlanService, this);
    ...
    bool AStar::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,
        ...
        Node start_node(gridStartX,gridStartY,0,0,0,grid2Index(gridStartX,gridStartY),0);
        Node goal_node(gridGoalX,gridGoalY,0,0,0,grid2Index(gridGoalX,gridGoalY),0);
        ...
        // ROS_INFO("Running A*");
        // planning
        found_legal = runAStar(costmap_->getCharMap(),start_node, goal_node, path, expand);
    ...
    bool AStar::runAStar(const unsigned char* global_costmap,const Node& start, const Node& goal, std::vector<Node>& path, std::vector<Node>& expand){
        path.clear();
        expand.clear();

        //open list and closed list
        auto comp = [](Node left, Node right){return left.f_ > right.f_;};
        ...
        while(!openList.empty()){
            ...
            for (const auto& action: actions){
                ...
                // update parent and heuristic 
                new_node.parent_ = current.index_;
                new_node.distance(goal);
                new_node.updateHeuristic();
    ...
    bool AStar::goalCheck(Node& node, const Node& goal){
        return (node.x_ == goal.x_ && node.y_ == goal.y_) ? true : false;
    }

    bool AStar::indexExistsInDict(std::unordered_map<int, Node>& closedList, Node &n){
    ...
    void AStar::publishExpand(std::vector<Node>& expand){
    ...
    void AStar::publishPlan(const std::vector<geometry_msgs::PoseStamped>& plan){
        if(!initialized_){
            ROS_ERROR("This planner has not been initialized.");
            return;
        }
    ...
    bool AStar::makePlanService(nav_msgs::GetPlan::Request& req, nav_msgs::GetPlan::Response& resp){
    ...
    bool AStar::world2Map(double worldX, double worldY, double& mapX, double& mapY){
    ...
    void AStar::map2World(double mapX, double mapY, double& worldX, double& worldY){
        worldX = originX + (mapX + convert_offset) * resolution;
        worldY = originY + (mapY + convert_offset) * resolution;
    }

    void AStar::map2Grid(double mapX, double mapY, int& gridX, int& gridY){
        gridX = (int)mapX;
        gridY = (int)mapY;
    }

    int AStar::grid2Index(int x, int y){
