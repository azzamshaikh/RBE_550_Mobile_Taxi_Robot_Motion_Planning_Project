...
namespace A_Star_Planner {
    ...
    bool AStar::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,
        ...
        // ROS_INFO("Running A*");
        // planning
        path_found = runAStar(costmap_->getCharMap(),start_node, goal_node, path, expand);
    ...
    std::vector<Node> AStar::convertClosedListToPath(std::unordered_map<int, Node>& closedList, const Node& start, 
                                                   const Node& goal){
        std::vector<Node> path;
    ...
    void AStar::outlineMap(unsigned char* costarr){
